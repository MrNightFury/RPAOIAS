...
export function assembleString(instruction: string): number {
    const parts = instruction.trim().split(" ");
    const mnemonic = parts[0].toUpperCase() as keyof typeof opcodes | keyof typeof longAddressOpcodes;
    // @ts-ignore
    const opcode = opcodes[mnemonic] ?? longAddressOpcodes[mnemonic];
    if (opcode === undefined) {
...
export function parseOperand(operand: string): number {
...
export function getLabels(code: string) {
    return code.split("\n")
...
export function splitInstructions(code: string): string[] | string {
